...
Original file is located at
    https://colab.research.google.com/drive/1s8HGXaku6IYj8GZs1uwO0TKPaNLNbZHW

**Paatufy**

To retrieve the top songs of the specified Tamil artists and identify the trending artist on Spotify for each year from 2010 to 2024, you'll need to follow several steps. I'll outline the procedure, which includes fetching data, performing analysis, and creating visualizations.

**Steps Overview:**



1.  Fetch Top Songs of Artists: Use the Spotify API to retrieve the top tracks of each artist.
2.  Analyze Popularity Over Time: Since Spotify does not directly provide historical trending data, you will need to simulate trending artist data based on available popularity data.
3.  Create Visualizations: Use libraries like matplotlib or seaborn to visualize trends.
4.  Save Data to CSV: Store the data in a CSV file for further analysis.

**Step 1: Fetch Top Songs for Each Artist**

You'll use the spotipy library to retrieve the top songs of the artists. For simplicity, here's the basic code structure to fetch the top tracks:
"""
